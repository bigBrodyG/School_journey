...
int main(void) {
    ...
    for (int i = 1; i <= max; i++) {
        ...
        for (int b = i + 1; b <= max; b++) {
            int sommaQuadrati = i * i + b * b;


            if (sommaQuadrati > max * max) {
              break;
            }


            double radice = sqrt(sommaQuadrati);
            int radiceintera = (int)radice;


            if (radice == (double)radiceintera && radiceintera <= max) {

                if (radiceintera * radiceintera == sommaQuadrati) {
                    printf("Terna: %d, %d, %d\n", i, b, radiceintera);
                }
            }
        }
